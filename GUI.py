...
from Class import Employee, Guest, Venue, Supplier, Event, EventType, save_data, load_data, JobTitle
import random
...
class ManagementSystemGUI:
    ...
    def display_main_menu(self):
        # First, hide all frames and toplevel windows before showing the main menu again.
        for widget in self.master.winfo_children():
            if isinstance(widget, tk.Toplevel):
                widget.destroy()  # This will destroy the Toplevel window
            else:
                widget.pack_forget()  # This will hide widgets managed by pack
    ...
    def add_venue(self):
        try:
            ...
            name_dropdown = ttk.Combobox(add_window, textvariable=name_var,
                                         values=["Venue A", "Venue B", "Venue C", "Venue D", "Custom"],
                                         state="readonly")
            ...
            tk.Button(add_window, text="Save Venue",
                      command=lambda: self.save_new_venue(next_venue_id, name_var.get(), address_entry.get(),
